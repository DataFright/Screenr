/**
 * @fileoverview Cypress Batch Performance Spec
 *
 * Measures click-to-results latency for:
 * - one ~1MB resume
 * - ten ~1MB resumes
 *
 * Also captures API timing headers and writes a markdown report artifact used
 * by the project performance summary.
 */
...
/**
 * Allows partial runs in CI/local debugging:
 * - batchScenario=single
 * - batchScenario=ten
 * - default: all
 */
...
/**
 * Reads binary PDF fixtures so they can be passed to selectFile payloads.
 */
...
describe('Batch grading performance', () => {
  before(() => {
    // The "ten only" mode appends to an existing report generated by the
    // single scenario run; skip header reset to preserve combined output.
  ...
  function runBatchScenario(scenarioName: string, filePaths: string[]) {
    ...
    loadResumeFixtures(filePaths).then((fixtures) => {
      ...
      cy.wait('@gradeRequest', { requestTimeout: 300000, responseTimeout: 300000 }).then((interception) => {
        // API route emits X-Screenr-Total-Ms; keep this as backend timing while
        // click-to-results duration captures full user-perceived latency.
      ...
      cy.get('[data-testid="graded-resume-score"]').then(($scores) => {
        // Results should be ranked high-to-low by overall score.
