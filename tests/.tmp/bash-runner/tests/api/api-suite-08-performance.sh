...
PERFORMANCE_DIR="$PERFORMANCE_FIXTURES_DIR"
...
run_full_batch_profile() {
    ...
    local header_file="$TMP_DIR/perf-headers-$(echo "$label" | tr ' /~' '---').txt"
    local body_file="$TMP_DIR/perf-body-$(echo "$label" | tr ' /~' '---').json"
    local curl_cmd=(curl -s -D "$header_file" -o "$body_file" -w "%{http_code}" -X POST "$BASE_URL/api/grade" -H "X-Test-Mode: true" -F "jobTitle=Software Engineer" -F "jobDescription=Looking for an experienced software engineer with strong system design, delivery, and mentoring skills")

    for pdf in "$fixture_dir"/*.pdf; do
        curl_cmd+=(-F "files=@$pdf")
    done

    local start_time=$(date +%s%N)
    local http_code
    http_code=$("${curl_cmd[@]}" --max-time "$max_time_seconds")
    local end_time=$(date +%s%N)
    local elapsed_ms=$(( (end_time - start_time) / 1000000 ))
    local server_total_ms
    server_total_ms=$(grep -i '^x-screenr-total-ms:' "$header_file" | awk '{print $2}' | tr -d '\r' || true)
    local result_count
...
# Test P.11: Full batch performance (10 x ~1MB)
echo -e "\n${CYAN}Test P.11: Full batch grading with 10 resumes at ~1MB each${NC}"
run_full_batch_profile "10 x ~1MB" "$PERFORMANCE_DIR/1mb" 10 240
